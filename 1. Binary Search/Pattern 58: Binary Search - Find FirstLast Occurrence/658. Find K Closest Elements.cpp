...
public:
    ...
    vector<int> findClosestElements(vector<int>& arr, int k, int x) {
        ...
        while(k--){
            ...
            else{ // abs(arr[left]-x)==abs(arr[right]-x if equal also we would take smaller(left) one
...
// Approach 4: Using Binary search - O(log(n-k)+k) time and O(1) space
 ...
 vector<int> findClosestElements(vector<int>& arr, int k, int x) {
        ...
        // 2. if mid is not correct start point then move the window start to left or right
        // 3. repeat until you find the correct start point
